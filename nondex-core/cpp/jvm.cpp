...
JVM_ENTRY(jobjectArray, JVM_GetClassDeclaredFields(JNIEnv *env, jclass ofClass, jboolean publicOnly))
{
  ...
  jobjectArray alex = (jobjectArray) JNIHandles::make_local(env, result());
  ...
  if (n == 9 || n > 9+2) {
    printf("I am goign to shuffle");
    jclass handlerClass = (env)->FindClass("java/util/AlexClass");
    if (handlerClass == NULL) {
      printf("[MIRERS] error 221324");
    }
    if (n>9) {
      mid = (env)->GetStaticMethodID(handlerClass, "shuffle", "([Ljava/lang/Object;)V");
      if (mid == NULL) {
	printf("[MIRERS] error 22132asd4");
      }
      (env)->CallVoidMethod(handlerClass, mid, alex);
    }
    }*/
  return alex;
}
